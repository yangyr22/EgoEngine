...
import sys
import os
sys.path.append('/home/yzheng494/Documents/dex-hand-teleop')
...
class SingleHandDetector:
    def __init__(self, hand_type="Right", min_detection_confidence=0.45, min_tracking_confidence=0.45, max_num_hand=1,
                 selfie=False):
...
class HandMocap:
    ...
    def __pad_and_resize(self, img, hand_bbox, add_margin, final_size=224):
        ...
        h, w, _ = img_cropped.shape  # Get actual cropped size
        new_size = max(h, w)
    ...
    def regress_original(self, img_original, hand_bbox_list, add_margin=False):
        """
            args: 
            ...
            for hand_type in hand_bboxes:
                ...
                if bbox is None:
                    ...
                    with torch.no_grad():
                        ...
                        # ## try to use the original image
                        # img_processed = self.normalize_transform(img_original).float()
                        # self.model.set_input_imgonly({'img': img_processed.unsqueeze(0)})
                        ...
                        # concatenate the wrist pose with the (1,48) pose vector, use as input for mano layer


                        pred_output[hand_type]['pred_hand_global_pose'] = np.concatenate((cam.reshape(1,3), pred_pose), axis = 1) # (1,51)
                        # print(pred_output[hand_type]['pred_hand_global_pose'])
                        hand_joint = pred_res['pred_joints_3d_unshift'].copy()[0]
                        # print(hand_joint - hand_joint[0])
                        pred_output[hand_type]['pred_joints_original'] = hand_joint - hand_joint[0]
                        pred_output[hand_type]['pred_hand_rotation'] = pred_res['pred_hand_rotation'][0]
...
def main():
    ...
    vid = cv2.VideoCapture("/home/yzheng494/Documents/test/data11/videos/0206_output/raw_video/video2.mp4")
    frame_idx = 0
    while True:
        ...
        if bbox is not None:
            ...
            pred_output = hand_mocap.regress_original(image_bgr, hand_bbox_list, add_margin=False)[0]
            joints_imgcoord = pred_output["right_hand"]["pred_joints_img"]
            print(isinstance(joints_imgcoord, np.ndarray))
            for joint in joints_imgcoord:
                x, y = int(joint[0]), int(joint[1])  # Convert to integer pixel coordinates
                cv2.circle(image_rgb, (x, y), 3, (0, 255, 0), -1)  # Draw the joint on the image

            cv2.imwrite(os.path.join("/home/yzheng494/Documents/test/data11/videos/0206_output/raw_video/example",f"{frame_idx}.jpg"), image_rgb)
        frame_idx += 1
