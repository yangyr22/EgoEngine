...
def main():
    ...
    parser.add_argument('--hand_type', type=str, choices=['left', 'right'], default='right', help='Specify which hand to track (left or right).')
    ...
    while True:
        ...
        if bbox is not None:
            ...
            last_pred = pred_output
        else:
            no_res_count += 1

            # If no hand is detected, use the last valid results
            if last_pred is not None:
                print(f"Warning: No hand detected in frame {frame_idx}, use previous result instead")
                pred_output = last_pred
                # pred_mesh_list = demo_utils.extract_mesh_from_output(pred_output)
                # visualizer = Visualizer(rendererType="opengl")
                ...
                print(f"Warning: No hand detected in frame {frame_idx}, and no previous valid results available.")
                # continue  # Skip this frame if no previous valid results
        
        frame_idx += 1
    
    print(no_res_count/frame_idx)
