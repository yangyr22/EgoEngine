...
def load_hand_joints_from_json(folder_path):
    """
    Load 3D hand joints from JSON files in a folder.
    ...
    """
    json_files = sorted([f for f in os.listdir(folder_path) if f.endswith('.json')])  # Sort for correct frame order
    all_joints = []
    all_translations = []
    ...
    for json_file in json_files:
        ...
        # Extract right-hand joints if available
        right_hand = data.get("right_hand", None)
        if right_hand and "pred_3d_joints" in right_hand:
            hand_joints = np.array(right_hand["pred_3d_joints"])  # (21, 3)
            translation = np.array(right_hand.get("tvec", [0, 0, 0]))  # Global translation (3,)

            all_joints.append(hand_joints)
            all_translations.append(translation)
    
    # Convert to numpy arrays
    all_joints = np.array(all_joints)  # Shape (T, 21, 3)
    all_translations = np.array(all_translations)  # Shape (T, 3)

    return {
        "pred_3d_joints": all_joints,
        "tvec": all_translations
    }
...
def test():
    ...
    json_folder = "/home/yzheng494/Documents/test/0214_data/videos/robot_output13/json_folder"  # Update this path

    # Load data
    hand_data = load_hand_joints_from_json(json_folder)

    # Visualize
    visualize_mano_3d_traj_video(hand_data, output_path="output.mp4", fps=30)
    ...
    # exit()
    visualize_mano_3d_traj_video(hand_data, output_path="mano.mp4", fps=30)
