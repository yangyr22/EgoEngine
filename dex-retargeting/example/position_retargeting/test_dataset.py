...
def customeddataset(json_path=""):
    """
    读取 JSON 文件，将 t 个时刻的 tvec、pred_3d_joints、rvec 连接起来，并转换为 numpy 数组后返回字典。

    参数:
        json_path (str): JSON 文件路径。

    返回:
        dict: 包含 tvec、pred_3d_joints 和 rvec 的 numpy 数组的字典。
    """
    # 打开并读取 JSON 文件
    with open(json_path, "r") as file:
        data = json.load(file)
    
    tvec_list = [np.array(item["tvec"]) for item in data]  # 每个 tvec 形状假设为 (3,)
    pred_3d_joints_list = [np.array(item["pred_3d_joints"]) for item in data]  # 每个 pred_3d_joints 形状为 (21, 3)
    rvec_list = [np.array(item["rvec"]) for item in data]  # 每个 rvec 形状假设为 (3,)

    # 将每个时刻的数组在时间维度上堆叠，形成 (T, ...) 的数组
