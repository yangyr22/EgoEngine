...
def setup_args(parser):
    ...
    parser.add_argument(
        ...
        help="Path to CSV file of per-frame uL,vL,uR,vR coordinates (one line per frame)."
    )
...
class RemoveAnythingVideo(nn.Module):
    def __init__(
        ...
        self.image_segmentor = self.build_image_segmentor(
            segmentor_target, **segmentor_build_args)
        self.video_segmentor = self.build_video_segmentor(
            segmentor_target, **segmentor_build_args)
    ...
    def build_image_segmentor(self, target="sam", **kwargs):
        assert target == "sam", "Only support sam now."
        # 调用 SAM2 的构建函数
        return build_sam2_model(**kwargs)

    def build_video_segmentor(self, target="sam", **kwargs):
        assert target == "sam", "Only support sam now."
        # 调用 SAM2 的构建函数
        return build_sam2_model_video(**kwargs)
    ...
    def forward_segmentor(self, img, point_coords=None, point_labels=None,
        ...
        self.image_segmentor.set_image(img)
        masks, scores, logits = self.image_segmentor.predict(
    ...
    def forward(
        ...
        video_masks = predict_masks_with_sam2_video(
            video_path=video_path,
            predictor=self.video_segmentor,
            frame_coords_left=frame_coords_left,
            frame_coords_right=frame_coords_right,
            dilate_kernel_size=dilate_kernel_size,
            propagate=True,
            save_seg=False,
            output_dir=str(frame_mask_dir),
        )
        # `video_masks` is an (T,2,H,W) array of left/right 0/1 masks

        # now pick one of the two channels, or combine them:
        T = video_masks.shape[0]
        all_mask = []
        for i in range(T):
            left  = video_masks[i, 0].astype(bool)
            right = video_masks[i, 1].astype(bool)
            combined = (left | right).astype(np.uint8)   
            all_mask.append(combined)
        all_frame = [ iio.imread(fp)       for fp in frame_ps ]
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        # all_box
        all_frame_rm_w_mask, all_mask = model(
            frame_ps, 0, point_coords, point_labels, frame_coords_left, frame_coords_right, args.input_video, 
            frame_mask_dir, key_frame_mask_idx, dilate_kernel_size
        )
