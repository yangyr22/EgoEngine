...
def launch_robot_server(args: Args):
    ...
    if args.robot == "sim_ur":
        ...
        elif args.robot == "panda_deoxys":
            from gello.robots.panda_deoxys import PandaRobot
            
            # robot = PandaRobot("JOINT_IMPEDANCE")
            # robot = PandaRobot("OSC_POSE")
            robot = PandaRobot("OSC_POSE", gripper_type="robotiq")
        elif args.robot == "panda_deoxys_simple":
            from gello.robots.panda_deoxys_simple import PandaRobot

            # robot = PandaRobot("JOINT_IMPEDANCE")
            # robot = PandaRobot("OSC_POSE")
            robot = PandaRobot("OSC_POSE", gripper_type="robotiq")
        elif args.robot == "bimanual_panda_deoxys":
            from gello.robots.panda_deoxys import PandaRobot
            
            _robot_l = PandaRobot("OSC_POSE", gripper_type="robotiq", robot_cfg_file="charmander.yml") # our left
            _robot_r = PandaRobot("OSC_POSE", gripper_type="franka", robot_cfg_file="charmander2.yml") # our right
