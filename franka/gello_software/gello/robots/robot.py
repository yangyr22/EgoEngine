...
class BimanualRobot(Robot):
    ...
    def _append_lists(self, left, right):
        ...
        result = tuple()
        for i_list in range(len(left)):
            result = (*result, left[i_list] + right[i_list])
        return result

    def command_joint_state(self, joint_state: np.ndarray) -> None:
        result_l = self._robot_l.command_joint_state(joint_state[: self._robot_l.num_dofs()])
        result_r = self._robot_r.command_joint_state(joint_state[self._robot_l.num_dofs() :])
        if result_l is not None:
            assert result_r is not None
            return self._append_lists(result_l, result_r)
