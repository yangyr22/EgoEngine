...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    ...
    # panda RL2 left (ours)
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT8ISNDP-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6, 7),
        joint_offsets=(
            3 * np.pi / 2,
            2 * np.pi / 2,
        ...
        joint_signs=(1, -1, 1, 1, 1, -1, 1),
        gripper_config=(8, 201.2453125, 159.4453125),
    ),
    # panda RL2 right (ours)
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT8J0SM3-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6, 7),
        joint_offsets=(2*np.pi/2, 2*np.pi/2, 2*np.pi/2, 3*np.pi/2, 4*np.pi/2, 3*np.pi/2, 2*np.pi/2),
        joint_signs=(1, -1, 1, 1, 1, -1, 1),
        gripper_config=(8, 201.2453125, 159.4453125),
    ),
    # Left UR
