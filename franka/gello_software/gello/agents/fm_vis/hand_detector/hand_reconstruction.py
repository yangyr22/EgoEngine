...
class OnlineHandCapture:
    ...
    def compute_tvec_from_depth(self, wrist_points, depth_map, bbox):
        """
        ...
        """
        x, y, w, h = bbox
        bbox_center_x = int(wrist_points[0])
        bbox_center_y = int(wrist_points[1])
    ...
    def hand_reconstruction_frame(self, frame, depth_map=None):
        ...
        if bbox is not None:
            ...
            object_points_unshift, _ = self.align_hand_coordinate(object_points)
            ...
            if self.depth_enabled and depth_map is not None:
                ...
                tvec = self.compute_tvec_from_depth(image_points[0], depth_map, bbox[0])
                # rvec = pred_output[hand_key]['pred_hand_rotation']
                rvec, _ = self.solve_PnP(object_points_unshift, image_points)
            else:
                image_points = pred_output[hand_key]["pred_joints_img"][:, :2]
