...
class RobotEnv:
    ...
    def step(self, joints: np.ndarray) -> Dict[str, Any]:
        ...
        action = self._robot.command_joint_state(joints)
        self._rate.sleep()
    ...
    def get_obs(self) -> Dict[str, Any]:
        ...
        for name, camera in self._camera_dict.items():
            ...
            observations[f"{name}_image"] = image
            observations[f"{name}_depth"] = depth
            # print(f"obtained {name} image of shape", image.shape, f"and {name} depth of shape", depth.shape)
        ...
        if "joint_velocities" in robot_obs:
            observations["joint_velocities"] = robot_obs["joint_velocities"]
        if "ee_pos_quat" in robot_obs:
            observations["ee_pos_quat"] = robot_obs["ee_pos_quat"]
        if "eef_pos" in robot_obs:
            observations["ee_pos"] = robot_obs["eef_pos"]
        if "eef_quat" in robot_obs:
            observations["ee_quat"] = robot_obs["eef_quat"]
